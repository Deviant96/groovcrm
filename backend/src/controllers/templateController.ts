...
export async function update(req: Request, res: Response, next: NextFunction) {
  try {
    res.json(await templateService.updateTemplate(req.params.id as string, req.body));
  } catch (err) {
    next(err);
  }
}
...
export async function preview(req: Request, res: Response, next: NextFunction) {
  try {
    const { message, company, instagram, website, phone, score } = req.body;
...
export async function generateWhatsApp(req: Request, res: Response, next: NextFunction) {
  try {
    res.json(await templateService.generateWhatsApp(req.body));
  } catch (err) {
    next(err);
  }
}
